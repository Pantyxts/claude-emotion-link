...
目前用的是 **Hiyori**（Cubism 4 免费模型），28 个可控参数，覆盖了 10 种基础表情。模型背后还有一条 Canvas 画的"能量尾巴"，颜色和摆动幅度会跟着情绪一起变。页面上方有一个**置顶情绪指示器**，始终悬浮在最上层，方便直播或录屏时观众也能看到当前情绪。
...
直接双击 `start.bat`（Windows）或运行 `./start.sh`（Mac/Linux），脚本会自动检查环境、启动服务、打开浏览器。
